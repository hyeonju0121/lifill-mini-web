			...
			<div class="form_table no_line">
				<form name="updateNoticeForm" id="updateNoticeForm" method="post" 
					...
					<div class="tr">
						...
						<div class="td">	
							<select name="bsubcategory" id="bsubcategory">
								<option value="member" <c:if test="${board.bsubcategory eq '회원정보'}">selected</c:if>>회원정보</option>
								<option value="order" <c:if test="${board.bsubcategory eq '주문/결제'}">selected</c:if>>주문/결제</option>
								<option value="exchange" <c:if test="${board.bsubcategory eq '교환/반품'}">selected</c:if>>교환/반품</option>
					...
					<div class="btn_big_wrap btn_size_fix mt60">
						<button type="button" onclick="location.href='faqList'" class="white btn_list">목록</button>
